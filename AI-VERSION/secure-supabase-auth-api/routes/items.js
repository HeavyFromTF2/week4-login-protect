...
// NOTE: every query below runs through req.supabase, a client scoped to
// the caller's access token (see middleware/auth.js). Row Level Security
// policies on the `items` table (auth.uid() = user_id) do the actual
// access control - the API never needs to add "WHERE user_id = ..." by
// hand, and could not read/write another user's rows even if it tried.
